trigger PreventDelete on Appointment__c (before delete) {
    ...
    for (Appointment__c record : Trigger.old) {
        // Prevent deletion if the user's profile is NOT System Administrator
        if (UserInfo.getProfileId() != sysAdminProfileId) {
            record.addError('You do not have permission to delete Appointment records.');
        }
    }
}